...
function generateCards() {
    ...
    // if the computer starts the game this starts the function for the computers move
    if (currentPlayer === 2) {
...
// adds the computers moves in the game
function computerMove() {
    // gets all unflipped cards
    const unflippedCards = Array.from(document.querySelectorAll(".card:not(.flipped)"));

    // chooses a card to flip and flips it
    const randomIndex = Math.floor(Math.random() * unflippedCards.length);
    const computerFirstCard = unflippedCards[randomIndex];
    computerFirstCard.classList.add("flipped");
    firstCard = computerFirstCard;

    // after a delay it gets the now unflipped cards, selects one and flips it, it then start the checking for match function
    setTimeout(() => {
        const unflippedCardsAfterFirstFlip = Array.from(document.querySelectorAll(".card:not(.flipped)"));
        const randomIndexAfterFirstFlip = Math.floor(Math.random() * unflippedCardsAfterFirstFlip.length);
        const computerSecondCard = unflippedCardsAfterFirstFlip[randomIndexAfterFirstFlip];

        computerSecondCard.classList.add("flipped");
        secondCard = computerSecondCard;
